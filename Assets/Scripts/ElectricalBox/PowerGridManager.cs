...
public class PowerGridManager : MonoBehaviour
{
    ...
    {
        // 전선 잇기 완료 후에는 입력 이벤트 방지
        if (powerController.IsComplete) return;
    ...
    {
        if (powerController.IsComplete) // 전선 잇기 완료 후에는 입력 이벤트 방지
        {
            _isDragging = false;
            return;
        }
        ...
        if (_isDragging)
        {
                ...
                _lastTile.Data.IsFixed && _lastTile.Data.WireColor == _activeColor)
            {
                ...
                if (_completedColors.Count == 4) // 모든 색상 연결 시
                {
                    powerController.OnCompletePuzzle();
                }
            }
            else
            {
        ...
        else if (_selectedTile != null) // 단순 클릭 시 스왑
        {
            WireTile targetTile = GetTileUnderPointer();
            if (targetTile != null) TrySwap(_selectedTile, targetTile);
        }
    ...
    void Update()
    {
        // 전선 잇기 완료 후 / 퍼즐 비활성화에는 입력 이벤트 방지
        if (powerController.IsComplete || !powerController.IsPuzzleStarted) return;
    ...
    private WireTile GetTileUnderPointer()
    {
        ...
        // UI 레이캐스트 실행
        PointerEventData pointerData = new PointerEventData(EventSystem.current) { position = pointerPos };
        List<RaycastResult> results = new List<RaycastResult>();
        raycaster.Raycast(pointerData, results);

        if (results.Count > 0)
            return results[0].gameObject.GetComponentInParent<WireTile>();
        return null;
    }
