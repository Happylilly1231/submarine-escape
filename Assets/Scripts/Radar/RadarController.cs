...
public class RadarController : MonoBehaviour
{
    ...
    private void UpdateMonsterPos(float timer)
    {
        ...
        // 실제 게임 상 위치(x, y, z)
        _monsterPos.x = radarNormPos.x * _radarMaxDistance;
        _monsterPos.y = radarNormPos.y * _radarMaxDistance;
        _monsterPos.z = radarNormPos.z * _radarMaxHeight;
    ...
    private void UpdateSubmarinePos(float timer)
    {
        ...
        // 실제 게임 상 위치(x, y, z)
        _submarine2Pos.x = radarNormPos.x * _radarMaxDistance;
        _submarine2Pos.y = radarNormPos.y * _radarMaxDistance;
        _submarine2Pos.z = radarNormPos.z * _radarMaxHeight;
    ...
    public void UpdateCurrentSelectPos(GameObject clicked, Vector2 currentPos)
    {
        ...
        else // 그 외 다른 위치 클릭 시
        {
            Vector2 currentRealSelectPos = new Vector2(
                _currentSelectPos.x / _radarRadius * _radarMaxDistance,
                _currentSelectPos.y / _radarRadius * _radarMaxDistance);
